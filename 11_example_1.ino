...
// global variables
float  dist_ema = _DIST_MIN;    // EMA filtered value
float  dist_prev = _DIST_MIN;   // previous valid distance (mm)
...
void loop() {
  ...
  if ((dist_raw == 0.0) || (dist_raw > _DIST_MAX) || (dist_raw < _DIST_MIN)) {
      dist_filtered = dist_prev;          // keep previous valid distance
      digitalWrite(PIN_LED, LOW);         // out of range → LED OFF
  } else {                                // within desired range
      dist_filtered = dist_raw;
      dist_prev = dist_raw;
      digitalWrite(PIN_LED, HIGH);        // valid → LED ON
  }
  ...
  // --- Servo control ---
  float angle = map(dist_ema, _DIST_MIN, _DIST_MAX, 0, 180);
